...
`isLowRamDevice()` makes `ZenithAndroidMemoryDecision.profile` resolve to the conservative `LegacyPhone` frame profile even when physical RAM alone would choose a larger profile. That profile is a decision output for the future Android shell/tab-construction lifecycle, not a runtime hot-swap performed by this bridge. `apply_android_memory_pressure_signal()` applies only the decision's pressure to the process controller; it does not mutate the device profile of already constructed tabs. A future shell that supports profile replacement must own that lifecycle explicitly. `isLowRamDevice()` is therefore a device-class signal, not a transient pressure event.
...
Thread ownership, Android snapshot retention, and application of `decision.profile` are integration responsibilities of the future Android shell; these pure-native tests intentionally do not pretend to certify a JNI lifecycle that does not yet exist.
