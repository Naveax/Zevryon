...
# Bogus-comment NUL => unexpected-null + U+FFFD.
text = replace_once(
    text,
...
# Quoted attribute-value NUL => unexpected-null + U+FFFD.
text = replace_once(
    text,
...
# Unquoted attribute-value NUL => unexpected-null + U+FFFD.
text = replace_once(
    text,
    '''            if (character == '\\0') {\n                return fail_data_tokenizer(\n                    error_,\n                    "HTML Data-tag tokenizer input preprocessing/NUL replacement is not implemented");\n            }\n            if (!ascii_byte(character)) {\n                return fail_data_tokenizer(\n                    error_,\n                    "HTML Data-tag tokenizer non-ASCII attribute-value authority is not implemented");\n            }\n            if (character == '&') {\n''',
    '''            if (character == '\\0') {\n                if (!emit_parse_error(*cursor, "unexpected-null-character") ||\n                    !append_bounded_bytes(\n                        value,\n                        kReplacementCharacterUtf8,\n                        "HTML Data-tag tokenizer attribute value")) {\n                    return false;\n                }\n                ++*cursor;\n                continue;\n            }\n            if (!ascii_byte(character)) {\n                return fail_data_tokenizer(\n                    error_,\n                    "HTML Data-tag tokenizer non-ASCII attribute-value authority is not implemented");\n            }\n            if (character == '&') {\n''',
...
# Attribute-name NUL => unexpected-null + U+FFFD.
text = replace_once(
    text,
...
# Tag-open NUL follows tag-open anything-else, then is reconsumed by Data.
text = replace_once(
    text,
...
# Tag-name NUL => unexpected-null + U+FFFD.
text = replace_once(
    text,
...
# Solidus recovery emits unexpected-solidus first, then reconsumes NUL as an attribute-name byte.
text = replace_once(
    text,
...
# Update the focused recovery boundary for tag-open NUL.
...
text = replace_once(text, old, new, "focused tag-open NUL regression")
path.write_text(text, encoding="utf-8")

# Admit only the tag-family Data-state NUL paths for this diagnostic.
...
text = replace_once(text, old, new, "tag-family NUL census admission")
path.write_text(text, encoding="utf-8")

print("applied robust tag-state NUL diagnostic")
