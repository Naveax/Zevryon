...
# Admit only the top-level Data-state U+0000 path. Markup/tag/comment/DOCTYPE
# NUL handling stays fail closed in its state-local code.
...
if old_guard != expected_guard:
    raise SystemExit("top-level Data NUL guard drifted")
...
'''
path.write_text(text[:guard_pos] + new_guard + text[next_state_pos:], encoding="utf-8")
...
# Corpus admission remains deliberately bounded to ordinary Data character text.
...
text = replace_once(
    ...
    "ordinary Data NUL documentation",
)
text = replace_once(
    text,
    ...
    "remaining NUL documentation",
)
path.write_text(text, encoding="utf-8")

print("applied bounded ordinary Data-text raw NUL production patch")
