...
def terminal_record(
    ...
    *,
    ...
    if status not in TERMINAL_STATES:
        raise ValueError(f"unknown competitor terminal state: {status}")
    if status == "success" and not runtime_identity:
...
def leadership_coverage(records: Iterable[dict[str, object]]) -> dict[str, object]:
    ...
    for record in records:
        ...
        if key not in _REGISTRY:
            raise ValueError(f"result references unknown competitor: {key}")
