...
def main() -> int:
    ...
    invalid_context = RawCaseResult(
        status="invalid",
        reason="evidence context failed: ValueError: malformed metadata",
    )
    invalid_record = terminalize_executable_case(
        chrome,
        invalid_context,
        None,
        payload_bytes=8192,
    )
    validate_terminal_record(invalid_record)
    require(invalid_record["status"] == "invalid", "context failure did not remain terminal")
    ...
    require_value_error(
        lambda: terminalize_executable_case(chrome, success, None, payload_bytes=8192),
        "success without evidence context was accepted",
    )
    require_value_error(
