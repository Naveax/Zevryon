...
''',
    '''    # Diagnostic admission: ordinary Data NUL plus markup-declaration
    # bogus-comment NUL. Normal comments, DOCTYPE and tag-family NUL remain
    # fail closed in this isolated slice.
    if "\\x00" in input_text:
        if state_name in {
            "PLAINTEXT state",
            "RCDATA state",
            ...
            return "input-preprocessing-nul"
''',
    "markup bogus-comment NUL diagnostic classifier",
)
path.write_text(text, encoding="utf-8")

print("applied markup bogus-comment NUL diagnostic patch")
