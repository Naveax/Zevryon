...
def launch_plan(competitor: str) -> PlaywrightLaunchPlan:
    ...
    if spec.adapter != "playwright" or spec.playwright_browser is None:
        raise ValueError(f"competitor is not supported by the Playwright adapter: {competitor}")
...
def launch_browser(playwright: Any, competitor: str) -> tuple[CompetitorSpec, Any, PlaywrightLaunchPlan]:
    spec = get_spec(competitor)
...
def runtime_identity(
    ...
    channel = plan.channel if plan.channel is not None else "bundled"
    return (
        f"{spec.canonical_name}; adapter=playwright; "
        f"browser_type={plan.browser_type}; channel={channel}; version={version}"
    )
