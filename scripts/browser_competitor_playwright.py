...
_UNAVAILABLE_LAUNCH_MARKERS = (
    "executable doesn't exist",
    "executable does not exist",
    "playwright install",
    "download new browsers",
    "browser distribution is not found",
    "browser distribution 'chrome' is not found",
    "browser distribution 'msedge' is not found",
)
...
def launch_failure_status(exc: BaseException) -> tuple[str, str]:
    reason = f"{type(exc).__name__}: {exc}".strip()
    lowered = reason.lower()
    if any(marker in lowered for marker in _UNAVAILABLE_LAUNCH_MARKERS):
