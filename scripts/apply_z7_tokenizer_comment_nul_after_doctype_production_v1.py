...
''',
    ...
    if "\\x00" in input_text:
        if state_name in {
            ...
            return "input-preprocessing-nul"
''',
    "stacked DOCTYPE/comment NUL classifier",
)
path.write_text(text, encoding="utf-8")
...
text = text.replace(
    "- raw NUL replacement/input-stream preprocessing outside the separately proven ordinary Data and DOCTYPE slices;",
    "- raw NUL replacement/input-stream preprocessing outside the separately proven ordinary Data, DOCTYPE, and normal-comment slices;",
    1,
)
path.write_text(text, encoding="utf-8")

print("applied stacked comment NUL production patch")
