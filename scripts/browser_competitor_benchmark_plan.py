...
def plan_benchmark_cases(
    ...
    return [_case_plan(spec, mode) for spec in specs for mode in requested_modes]
...
def unsupported_case_record(
    ...
    *,
    ...
    if (
        spec.canonical_name != plan.canonical_name
        or spec.canonical is not plan.canonical
        or spec.adapter != plan.adapter
    ):
        raise ValueError("benchmark plan identity drifted from competitor registry")

    record = terminal_record(spec, "unsupported", reason=plan.reason)
    return {
        **record,
