...
EXPECTED_Z2R3C_HEAD = "0fb78d6f598f96eff1e9df6fde6897442e0ac443"
...
def validate_platform(
    ...
    require(
        manifest.get("slice") == "Z2R-3D-massivedoc-codec-promotion",
        f"{platform_name} slice mismatch",
    )
    ...
    require(
        authority.get("authoritative_backend") == "cpp",
        f"{platform_name} C++ authority lost",
    )
    require(
        authority.get("shadow_backend") == "rust",
        f"{platform_name} Rust shadow missing",
    )
    require(
    ...
    require(
        parity.get("fault_classes") == EXPECTED_FAULT_CLASSES,
        f"{platform_name} fault class contract mismatch",
    )
    require(
        len(str(parity.get("payload_sha256", ""))) == 64,
        f"{platform_name} payload SHA invalid",
    )
    require(
        len(str(parity.get("store_tree_sha256", ""))) == 64,
        f"{platform_name} tree SHA invalid",
    )

    operations = manifest.get("operations")
    require(
        isinstance(operations, list) and len(operations) == 4,
        f"{platform_name} operation set incomplete",
    )
    require(
    ...
    require(
        all(item.get("passed") is True for item in operations),
        f"{platform_name} operation failed",
    )
    require(
        manifest.get("total_p50_wall_seconds", {}).get("passed") is True,
        f"{platform_name} total gate failed",
    )
...
def build_final_manifest(
    ...
    *,
    ...
    require(is_commit_sha(commit_sha), "final commit SHA is invalid")
    require(
        z2r3c_head == EXPECTED_Z2R3C_HEAD,
        "Z2R-3C prerequisite SHA mismatch",
    )
    require(set(manifests) == set(PLATFORMS), "platform manifest set mismatch")
    validated = {
    ...
    for platform_name in PLATFORMS[1:]:
        ...
        require(
            ...
            f"{platform_name} parameters differ",
        )
        require(
            manifest["parity"]["payload_sha256"] == expected_payload,
            f"{platform_name} payload differs",
        )
        require(
            manifest["parity"]["store_tree_sha256"] == expected_tree,
            f"{platform_name} store tree differs",
        )
        require(
            manifest["parity"]["store_file_count"] == expected_files,
            f"{platform_name} file count differs",
        )
        semantics = {
    ...
    final: dict[str, Any] = {
        ...
        "platforms": {
            platform_name: {
                ...
                "memory_metric": validated[platform_name]["operations"][0]["memory"][
                    "metric"
                ],
                "slice_ready": True,
            }
