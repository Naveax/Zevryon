    ...
    *,
    ...
    for plan in ordered:
        ...
        try:
            ...
            try:
                if not isinstance(raw, RawCaseResult):
                    raise ValueError("case runner did not return RawCaseResult")
                raw.validate()
            except ValueError as exc:
                raw = RawCaseResult(
                    status="invalid",
                    reason=_exception_reason("case runner returned invalid result", exc),
                )
