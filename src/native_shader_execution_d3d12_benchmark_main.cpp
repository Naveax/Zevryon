...
int main(int argc, char** argv) {
    ...
    if (owner == nullptr || !owner->initialize(make_sdk_config(), &sdk_error)) {
        std::cerr << sdk_error.message << '\n';
        return 7;
    }
    const NativeGpuSdkProbe probe = owner->probe();
    ...
    config.executor_generation = 211U;
    ...
    std::cout << "schema=zevryon.d3d12-integer-shader-execution.v1\n";
    std::cout << "iterations=" << iterations << '\n';
    std::cout << "samples=" << samples.size() << '\n';
    std::cout << "hardware_device="
              << (((probe.flags & kNativeGpuSdkSoftwareDevice) == 0U) ? 1 : 0)
              << '\n';
