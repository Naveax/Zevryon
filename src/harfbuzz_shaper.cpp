...
hb_direction_t hb_direction(ShapingDirection direction) noexcept {
    switch (direction) {
...
bool validate_request(
    ...
    if (static_cast<std::size_t>(request.cluster_limit) >
        static_cast<std::size_t>(
            std::numeric_limits<std::uint32_t>::max())) {
        return fail(
            HarfBuzzShapingErrorKind::IndexOverflow,
            request.cluster_limit,
            "shaping cluster indices exceed the 32-bit contract",
            error);
    }
    ...
    *codepoint_limit = request.grapheme_boundaries[
    ...
    if (*codepoint_limit - *first_codepoint >
        ...
        return fail(
            HarfBuzzShapingErrorKind::IndexOverflow,
            request.first_cluster,
            "shaping segment exceeds the HarfBuzz buffer index contract",
            error);
    }
    ...
    const std::size_t script_index = static_cast<std::size_t>(request.script);
    if (script_index >= static_cast<std::size_t>(ScriptId::Count) ||
    ...
    *stats = {};
    ...
    try {
        HbBlob blob(hb_blob_create(
            ...
            nullptr,
            nullptr));
        if (!blob || hb_blob_get_length(blob.get()) != request.font_bytes.size()) {
            return fail(
        ...
        hb_buffer_set_direction(buffer.get(), hb_direction(request.direction));
...
#ifdef HB_BUFFER_FLAG_PRODUCE_UNSAFE_TO_CONCAT
        if (request.produce_unsafe_to_concat) {
            buffer_flags = static_cast<hb_buffer_flags_t>(
                buffer_flags | HB_BUFFER_FLAG_PRODUCE_UNSAFE_TO_CONCAT);
        }
#endif
#ifdef HB_BUFFER_FLAG_PRODUCE_SAFE_TO_INSERT_TATWEEL
        buffer_flags = static_cast<hb_buffer_flags_t>(
        ...
        for (std::uint32_t cluster = request.first_cluster;
            ...
            const std::size_t cluster_limit =
        ...
        const hb_bool_t shaped = hb_shape_full(
            font.get(),
            buffer.get(),
            features.empty() ? nullptr : features.data(),
            static_cast<unsigned int>(features.size()),
            nullptr);
        if (shaped == 0 || !hb_buffer_allocation_successful(buffer.get())) {
            return fail(
