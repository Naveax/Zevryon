...
bool import_logical_node_source_v2_to_arena_v2(
    ...
    // The bound writer independently re-inspects the native store, verifies the
    // payload SHA, and freezes payload + record-sequence + record-count identity
    // into the published v2 arena. Source validation above and destination
    // binding below are therefore independent authority checks.
