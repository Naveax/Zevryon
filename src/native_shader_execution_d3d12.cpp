...
public:
    ...
    bool configure(
        ...
        snapshot_.capability_flags =
            kNativeShaderExecutionIntegerComposition | kNativeShaderExecutionPersistentAtlas |
            kNativeShaderExecutionGpuReadback | kNativeShaderExecutionRetainedContext |
    ...
    bool execute(
        ...
        try {
            ...
            if (readback != nullptr) {
                ...
                to_shader.Transition.StateBefore = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
                to_shader.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
                command_list_->ResourceBarrier(1U, &to_shader);
            }
            ...
            if (readback != nullptr) {
                ...
                for (std::uint32_t row = 0U; row < candidate.height; ++row) {
                    std::memcpy(candidate.bgra.data() +
            ...
            last_surface_.flags = kNativeShaderSurfaceReady | kNativeShaderSurfaceNonOwning |
            ...
            last_surface_.native_resource =
                static_cast<std::uint64_t>(reinterpret_cast<std::uintptr_t>(output_texture_.Get()));
            ...
            snapshot_.peak_transient_bytes = std::max(snapshot_.peak_transient_bytes, transient);
            return true;
    ...
    bool export_surface(NativeShaderSurfaceView* surface,
        ...
        if (!native_shader_surface_view_valid(last_surface_) || output_texture_ == nullptr) {
            *surface = {};
...
private:
    ...
    bool ensure_output_locked(
        ...
        if (next_output_generation_ == (std::numeric_limits<std::uint64_t>::max)()) {
            return fail(error, NativeShaderExecutionErrorKind::ResourceAllocationFailed,
