...
struct OperationCounts {
    ...
    std::uint64_t total() const noexcept {
        return insert + erase + move + update_height + update_summary;
    }

    bool all_exercised() const noexcept {
        return insert > 0U && erase > 0U && move > 0U &&
               update_height > 0U && update_summary > 0U;
    }
};
...
struct VerificationReceipt {
    ...
    std::uint64_t logical_order_mismatches{0};
};
...
std::string report_json(
    ...
    const bool count_sum_matches = counts.total() == receipt.operations_completed;
    const bool all_classes = counts.all_exercised();
    const bool threshold_met = receipt.operations_completed >= kCertificationOperations;
    const bool certification_eligible =
        config.certification && threshold_met && all_classes && integrity_passed && count_sum_matches;
    const bool gate_passed = integrity_passed && count_sum_matches && all_classes &&
    ...
    stream << "{\n"
           ...
           << "  \"operations_completed\": " << receipt.operations_completed << ",\n"
           ...
           << "  },\n"
           << "  \"operation_count_sum_matches\": " << (count_sum_matches ? "true" : "false") << ",\n"
           << "  \"all_mutation_classes_exercised\": " << (all_classes ? "true" : "false") << ",\n"
           ...
           << "  \"logical_order_mismatches\": " << receipt.logical_order_mismatches << ",\n"
           << "  \"integrity_mismatches\": " << receipt.integrity_mismatches << ",\n"
           << "  \"failure_reason\": ";
...
void mark_integrity_failure(VerificationReceipt* receipt, std::string_view error) {
    if (receipt == nullptr) {
        return;
    }
    ++receipt->integrity_mismatches;
    if (error.find("logical order") != std::string_view::npos ||
        error.find("prefix") != std::string_view::npos) {
        ++receipt->logical_order_mismatches;
    }
}
...
bool run_mutations(
    ...
    for (std::size_t index = 0U; index < kInitialRecords; ++index) {
        ...
        if (!sequence->insert(static_cast<std::uint64_t>(oracle.size()), record, error)) {
            mark_integrity_failure(receipt, *error);
            return false;
        }
    ...
    for (std::uint64_t iteration = 0U; iteration < config.operations; ++iteration) {
        const std::uint64_t action = random() % 10U;
        ...
        ++receipt->operations_completed;

        if (receipt->operations_completed % kCheckpointInterval == 0U) {
...
int main(int argc, char** argv) {
    ...
    OperationCounts counts;
    VerificationReceipt receipt;
    const bool integrity_passed = run_mutations(config, &counts, &receipt, &sequence, &error);

    if (integrity_passed && !counts.all_exercised()) {
        error = "not every mutation class was exercised";
        mark_integrity_failure(&receipt, error);
    }
    if (integrity_passed && counts.total() != receipt.operations_completed) {
        error = "operation counts do not sum to operations_completed";
        mark_integrity_failure(&receipt, error);
    }

    const bool final_passed = integrity_passed && receipt.integrity_mismatches == 0U;
    const auto report = report_json(config, counts, receipt, sequence, final_passed, final_passed ? std::string_view{} : std::string_view(error));
    std::string report_error;
    if (!write_report(config, report, &report_error)) {
        std::cerr << "M8 mixed-mutation report failure: " << report_error << '\n';
        return 1;
    }
