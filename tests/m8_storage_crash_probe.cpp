...
GenerationPublicationCut publication_cut(std::string_view name, bool* valid) {
    *valid = true;
    if (name == "after-payload-flush") {
        return GenerationPublicationCut::after_payload_flush;
    }
    if (name == "after-prepare") {
        return GenerationPublicationCut::after_prepare;
    }
    if (name == "after-manifest-temp") {
        return GenerationPublicationCut::after_manifest_temp;
    }
    if (name == "after-manifest") {
        return GenerationPublicationCut::after_manifest;
    }
    if (name == "after-commit") {
        return GenerationPublicationCut::after_commit;
    }
    *valid = false;
...
GenerationCompactionCut compaction_cut(std::string_view name, bool* valid) {
    *valid = true;
    if (name == "after-journal-temp") {
        return GenerationCompactionCut::after_journal_temp;
    }
    if (name == "after-journal-replace") {
        return GenerationCompactionCut::after_journal_replace;
    }
    if (name == "after-stale-quarantine") {
        return GenerationCompactionCut::after_stale_quarantine;
    }
    *valid = false;
...
int command_compact(
    ...
    std::string error;
    if (!zevryon::massivedoc::compact_store_generation_metadata(
            root,
...
int command_recover(const std::filesystem::path& root) {
    ...
    std::cout << "{\"protocol_present\":"
              << (recovery.protocol_present ? "true" : "false")
    ...
    if (recovery.authority_manifest.empty()) {
        ...
        std::cout << static_cast<unsigned int>(
            std::to_integer<unsigned char>(recovery.authority_manifest.front()));
    }
    std::cout << ",\"segments\":[";
...
void usage() {
    std::cerr
        << "usage: m8_storage_crash_probe <seed|crash-publish|publish|crash-compact|compact|recover> ...\n";
}
...
int main(int argc, char** argv) {
    ...
    if (command == "crash-publish" || command == "publish") {
        ...
        bool valid = false;
        const auto cut = command == "publish"
            ? GenerationPublicationCut::none
            : publication_cut(argv[4], &valid);
        if (command == "publish") {
            valid = std::string_view(argv[4]) == "none";
        }
        if (!valid) {
    ...
    if (command == "crash-compact" || command == "compact") {
        ...
        bool valid = false;
        const auto cut = command == "compact"
            ? GenerationCompactionCut::none
            : compaction_cut(argv[3], &valid);
        if (command == "compact") {
            valid = std::string_view(argv[3]) == "none";
        }
        if (!valid) {
