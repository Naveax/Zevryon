...
def test_z3_css_at_rule_recovery_foundation_scope() -> None:
    ...
    assert "bool parse_at_rule(" in source
    assert "CssAtRuleContextV1 context" in source
    assert "std::uint32_t owner_rule_index" in source
    assert "kCssAtRuleNoOwnerV1" in source
