...
bool direct_faces_share_content_resource(const TemporaryDirectory& temporary) {
    ...
    bool ok = expect(resolve_catalog_font_resource(
                         fixture.generation,
                         fontconfig_face,
                         font.size(),
                         &cache,
    ...
    ok &= expect(first &&
                     first_stats.generation_id == 77U &&
                     first_stats.face_id == fontconfig_face &&
                     first_stats.platform_kind ==
    ...
    ok &= expect(first_stats.file_load.staging.current_bytes == 0U &&
    ...
    ok &= expect(resolve_catalog_font_resource(
                         fixture.generation,
                         directwrite_face,
                         font.size(),
                         &cache,
    ...
    ok &= expect(second == first &&
                     second_stats.platform_kind ==
                         FontPlatformIdentityKind::DirectWrite &&
    ...
    ok &= expect(second_stats.identity_bytes == directwrite.size() &&
...
bool unsupported_capabilities_fail_before_io(
    ...
    ok &= expect(!output &&
                     error.kind ==
                         CatalogFontResourceErrorKind::IdentityParseFailed &&
    ...
    ok &= expect(!output &&
                     error.kind == CatalogFontResourceErrorKind::PathTooLong &&
...
bool nested_file_failure_and_atomic_output(const TemporaryDirectory& temporary) {
    ...
    const std::string identity = directwrite_identity(
        {path_utf8(missing)}, "0", "MissingPS");
    GenerationFixture fixture;
    ...
    output = std::make_shared<VerifiedFontResource>(1U, 1U);
    ok &= expect(!resolve_catalog_font_resource(
                         fixture.generation,
                         kInvalidFontFaceId,
                         1024U,
