...
int main() {
    ...
    auto host = make_metal_window_test_host(640U, 360U);
    assert(host != nullptr);
    ...
    assert(owner->initialize(owner_config(window), &sdk_error));
    ...
    NativeShaderExecutionConfig stale = config;
    stale.context.runtime_generation += 1U;
    std::unique_ptr<NativeShaderExecutor> stale_executor =
        make_metal_native_shader_executor();
    assert(stale_executor != nullptr);
    assert(!stale_executor->configure(stale, &error));
    assert(error.kind == NativeShaderExecutionErrorKind::NativeContextUnavailable ||
           error.kind == NativeShaderExecutionErrorKind::StaleGeneration);
