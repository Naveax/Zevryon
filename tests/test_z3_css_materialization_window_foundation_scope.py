...
def test_z3_css_materialization_window_foundation_scope() -> None:
    ...
    assert "selection is one contiguous subspan and does not scan unrelated document nodes" in scope[
        "selection_model"
    ]
    ...
    assert "materialize_css_style_window_v1" in header
    assert "offscreen_before_nodes" in header
    assert "offscreen_after_nodes" in header
    assert "document_terminal_nodes.subspan" in source
