...
bool build(
    ...
    if (!build_directwrite_generation(
        ...
        std::cerr << "DirectWrite discovery failed: "
                  ...
                  << " family=" << error.family_index
                  << " font=" << error.font_index
...
bool validate_generation(
    ...
    const DirectWriteDiscoveryStats& stats) {
    bool ok = true;
    const auto records = generation.discovery_records();
    const auto& catalog = generation.catalog();
    ok &= expect(!records.empty(), "system DirectWrite enumeration must not be empty");
    ok &= expect(
    ...
    ok &= expect(
        stats.fonts_seen >= stats.faces_emitted + stats.simulated_fonts_skipped,
        "physical and simulated counts cannot exceed enumerated fonts");
    ok &= expect(stats.families_seen > 0U, "system collection must contain families");
    ok &= expect(stats.coverage_codepoints > 0U, "coverage scan must see codepoints");
    ...
    for (std::size_t index = 0U; index < records.size(); ++index) {
        ...
        ok &= expect(
            ...
            "every DirectWrite system face must carry the system flag");
        ...
        if (first <= catalog.coverage_ranges.size() &&
            ...
            for (std::size_t offset = 0U; offset < count; ++offset) {
                ...
                ok &= expect(
                    ...
                    "DirectWrite Unicode ranges must contain valid scalar bounds");
                if (offset != 0U) {
...
int main() {
    ...
    ok &= expect(
        ...
        "unchanged DirectWrite state must reproduce the same fingerprint");
    ok &= expect(
        equal_ranges(first->discovery_records(), second->discovery_records()),
        "unchanged DirectWrite state must reproduce discovery records");
    ok &= expect(
        equal_ranges(first->families(), second->families()),
        "unchanged DirectWrite state must reproduce family records");
    ok &= expect(
        first->catalog().faces == second->catalog().faces,
        "unchanged DirectWrite state must reproduce catalog faces");
    ok &= expect(
        first->catalog().coverage_ranges == second->catalog().coverage_ranges,
        "unchanged DirectWrite state must reproduce coverage");
    ok &= expect(
        first->catalog().script_buckets == second->catalog().script_buckets,
        "unchanged DirectWrite state must reproduce script buckets");
    ok &= expect(
        first->catalog().script_face_ids == second->catalog().script_face_ids,
        "unchanged DirectWrite state must reproduce script face IDs");
    ...
    std::cout << "DirectWrite discovery tests passed: families="
              << first_stats.families_seen
              ...
              << " simulated=" << first_stats.simulated_fonts_skipped
