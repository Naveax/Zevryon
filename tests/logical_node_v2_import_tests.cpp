...
bool test_cross_record_import_round_trip() {
    ...
    if (!require(arena.open(&error), error) ||
        !require(arena.source_binding().source_record_count == binding.source_record_count,
                 "imported arena binds exact physical record count") ||
        !require(arena.source_binding().payload_sha256 == binding.payload_sha256,
                 "imported arena binds exact payload SHA") ||
        !require(arena.source_binding().record_sequence_sha256 == binding.record_sequence_sha256,
                 "imported arena binds exact physical record sequence") ||
